...
test.describe("Checkout", () => {
    test("user can successfully place an order", async ({ page }) => {
        ...
        await expect(
            page.getByText("AMD Ryzen 7 9800X3D")
        ).toBeVisible();

        await expect(
            page.getByText("NVIDIA RTX 5070")
        ).toBeVisible();
    ...
    test("user sees validation errors when required fields are empty", async ({ page }) => {
        ...
        await expect(
            page.getByText("Postcode is required")
        ).toBeVisible();
        ...
        await expect(
            page.getByText("Country is required")
        ).toBeVisible();
